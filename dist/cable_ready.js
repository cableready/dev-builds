...
class UpdatesForElement extends SubscribingElement {
  ...
  async update(data) {
    ...
    if (blocks.length === 0) {
      this.triggerElementLog.push(Log.cancel(this.lastUpdateTimestamp, "All elements filtered out"));
      return;
    }
    // first <cable-ready-updates-for> element in the DOM *at any given moment* updates all of the others
        if (blocks[0].element !== this) {
      this.triggerElementLog.push(Log.cancel(this.lastUpdateTimestamp, "Update already requested"));
      return;
    }
    ...
    this.triggerElementLog.push(Log.response(this.lastUpdateTimestamp, this, uniqueUrls));
...
class Block {
  ...
  async process(data, html, fragmentsIndex, startTimestamp) {
    ...
    this.element.targetElementLog.push(Log.morphStart(startTimestamp, this.element));
    ...
    this.element.targetElementLog.push(Log.morphEnd(startTimestamp, this.element));
  }
