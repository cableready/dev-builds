...
const pluginStore = new Proxy(plugins, {
  get: (target, name) => name in target ? target[name] : () => {
    console.error(`CableReady: No plugin registered for ${name}. Make sure to call CableReady.registerPlugin during initialization. For example:\n\nimport CableReady from 'cable_ready'\nimport ${name} from '${name}'\nCableReady.registerPlugin('${name}', ${name})`);
  }
});
  ...
  morph: operation => {
    processElements(operation, (element => {
      ...
      operate(operation, (() => {
        ...
        pluginStore.morphdom(element, childrenOnly ? template.content : template.innerHTML, {
      ...
      console.warn('The "morph" operation is deprecated in CableReady v5 and will be removed in v6. Use "morphdom" or similar instead.');
      after(parent ? parent.children[idx] : document.documentElement, operation);
    }));
  },
  morphdom: operation => {
...
const add = newOperations => {
  window.CableReadyOperationsStore = {
    ...window.CableReadyOperationsStore,
    ...newOperations
  };
};
...
class Block {
  ...
  async process(data, html, index) {
    ...
    pluginStore.morphdom(this.element, fragments[blockIndex], {
      childrenOnly: true,
      onBeforeElUpdated: shouldMorph(operation),
      onElUpdated: _ => {
  ...
  async resolveTurboFrames(docFragment) {
    const reloadingTurboFrames = [ ...docFragment.querySelectorAll('turbo-frame[src]:not([loading="lazy"])') ];
...
register("morphdom", morphdom);

const initialize = (initializeOptions = {}) => {
  const {consumer: consumer, onMissingElement: onMissingElement, plugins: plugins} = initializeOptions;
  if (consumer) {
  ...
  if (plugins) Object.keys(plugins).forEach((plugin => register(plugin, plugins[plugin])));
  defineElements();
};
...
export { CableReadyElement, morph_callbacks as MorphCallbacks, plugins$1 as Plugins, StreamFromElement, SubscribingElement, UpdatesForElement, utils as Utils, global as default };
